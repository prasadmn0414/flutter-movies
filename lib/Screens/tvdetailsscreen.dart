...
import 'package:flutter_movie_app/Network/tvdetail.dart';
import 'dart:convert';
...
class _MainCollapsingToolbarState extends State<TVDetailScreen> {
  double offset = 0.0;

  ScrollController _scrollController;
  TVDetails tvDetails;

  Future<void> _getTVDetails;
  ...
  Future<void> getTVDetails() async {
    int tvid = widget.tvid;
    String url =
        'https://api.themoviedb.org/3/tv/$tvid?api_key=$apikey&append_to_response=videos,credits,similar';
    final movieDetailResponse = await http.get(url);
    if (movieDetailResponse.statusCode == 200) {
      // If the call to the server was successful, parse the JSON
      var decRes = jsonDecode(movieDetailResponse.body);
      tvDetails = TVDetails.fromJson(decRes);
    } else {
      ...
      headerSliverBuilder: (BuildContext context, bool innerBoxIsScrolled) {
        return <Widget>[
          SliverAppBar(
            ...
            floating: false,
            pinned: true,
            title: _showTitle ? Text(tvDetails.name) : null,
                          ...
                          Flexible(
                            child: Column(
                              ...
                              children: <Widget>[
                                ...
                                Text(getGenres(tvDetails.genres),
                                    maxLines: 2,
                                    overflow: TextOverflow.ellipsis,
                                    style: TextStyle(
                                      color: Colors.white,
        ...
        children: <Widget>[
                ...
                Row(
                  children: <Widget>[
                    ...
                    Column(
                      ...
                      children: <Widget>[
                        Text(
                          dateTimeFormat(tvDetails.firstAirDate),
                          style: TextStyle(
                              color: Colors.grey[600],
                              fontSize: 15.0,
                              fontWeight: FontWeight.normal),
                        ...
                        SizedBox(height: 5.0,),
                        Text(
                          "${(tvDetails.episodeRunTime[0])} min(s)",
                          style: TextStyle(
                              color: Colors.grey[600],
                              fontSize: 15.0,
                              fontWeight: FontWeight.normal),
          ...
          Container(
            ...
            child: ListView.builder(
                ...
                primary: false,
                itemCount: tvDetails.videos.results.length,
                itemBuilder: (BuildContext context, int index) {
                  return getTrailerCard(tvDetails.videos.results[index]);
                }),
          ),
          Divider(
            color: Colors.grey,
          ...
          Container(
            ...
            child: ListView.builder(
                ...
                physics: ClampingScrollPhysics(),
                primary: false,
                itemCount: tvDetails.credits.cast.length,
                itemBuilder: (BuildContext context, int index) {
                  return getCastCard(tvDetails.credits.cast[index]);
                }),
          ),
          Padding(
            padding: const EdgeInsets.only(
          ...
          Container(
            ...
            child: ListView.builder(
                ...
                physics: ClampingScrollPhysics(),
                primary: false,
                itemCount: tvDetails.similar.results.length,
                itemBuilder: (BuildContext context, int index) {
                  return getSmallCard(tvDetails.similar.results[index]);
                }),
          ),
          SizedBox(
            height: 10.0,
  ...
  Widget getSmallCard(SimilarResults tv) {
    ...
    return InkWell(
      ...
      onTap: () {
        Navigator.push(context,
            MaterialPageRoute(builder: (context) => TVDetailScreen(tv.id)));
      },
            ...
            Container(
              ...
              child: ClipRRect(
                borderRadius: BorderRadius.only(
                    ...
                    topRight: Radius.circular(5.0)),
                child: CachedNetworkImage(
                  imageUrl: "https://image.tmdb.org/t/p/w342" + tv.posterPath,
                  fit: BoxFit.cover,
                ),
              ),
            ),
